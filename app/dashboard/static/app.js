...
function overview() {
  ...
  const gaps = state.data.capability_gaps || [];
  const gapPanel = gaps.length ? `<section class="panel capability-gap"><div><p class="eyebrow">CAPABILITY DISCOVERY</p><h2>${gaps.length} requested capability ${gaps.length === 1 ? 'is' : 'are'} not available locally</h2><p class="muted">The original mission is safely paused while a separate constrained mission researches free/open-source options. Nothing is installed or delegated automatically.</p></div>${gaps.map(gap => `<div class="gap-row"><span>${status(gap.status)}</span><b>${esc(gap.domain.replaceAll('_', ' '))}</b><small>Missing: ${esc((gap.missing_tools || []).join(', ') || 'specialist agent')}</small></div>`).join('')}</section>` : '';
  return `${quickStart}${gapPanel}<div class="cards">${cards}</div><div class="grid"><section class="panel"><div class="panel-header"><h2>Live activity</h2><small>${events.length} retained events</small></div><div class="timeline">${activity}</div></section><section class="panel"><div class="panel-header"><h2>Component health</h2><small>Last snapshot</small></div>${components}</section></div>`;
}
