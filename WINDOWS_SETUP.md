...
partial, and unavailable integrations. Win32 `ctypes` supplies opt-in keyboard/mouse polling and PowerShell
supplies PnP device, AudioEndpoint, and session inventory. These sensitive observers remain disabled by
default; a missing API is isolated by `EventManager` and the rest of the application continues.
...
requested or bypassed automatically. Power actions require both `power.control` and `system.control` and
are always high-risk, so they cannot run without explicit confirmation.
